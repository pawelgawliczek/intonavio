"""BullMQ worker wrapper with heartbeat for stem-split jobs."""
...
async def run_heartbeat(config: WorkerConfig, stop_event: asyncio.Event) -> None:
...
async def create_worker(
    ...
    """Create a BullMQ worker listening on the stem-split queue."""
    worker = Worker(
        config.queue_name,
        handler,
