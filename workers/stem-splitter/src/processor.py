...
from src.db import complete_stem_split, create_connection, mark_song_splitting
...
def _stem_key(song_id: str, stem_type: str) -> str:
    return f"stems/{song_id}/{stem_type}.mp3"
...
async def process_stem_split_job(
    ...
    Steps:
        1. Mark Song as SPLITTING
        2. If `stems/{songId}/FULL.mp3` already exists in R2 → reuse (re-process path)
           Otherwise download via yt-dlp and upload as FULL.mp3
        3. Run BS-Roformer separation → vocals + instrumental
        4. Upload VOCALS.mp3 and OTHER.mp3 to R2
        5. Upsert 3 Stem rows (FULL/VOCALS/OTHER), transition Song -> ANALYZING
        6. Enqueue pitch-analysis job
    """
    ...
    # 1. Mark Song SPLITTING (idempotent)
    ...
    full_key = _stem_key(song_id, "FULL")
    vocals_key = _stem_key(song_id, "VOCALS")
    other_key = _stem_key(song_id, "OTHER")
