        ...
        .sheet(isPresented: $isShowingProgress) {
            if let vm = viewModel {
                ProgressLogView(
                    ...
                    onSwitchVariant: { variant in
                        vm.switchToVariant(variant, apiClient: APIClient())
                    },
...
private extension SongPracticeView {
    ...
    func refreshSongIfOnline() {
        guard !isOffline, let vm = viewModel else { return }
        Task { @MainActor in
            do {
                let updated = try await APIClient().getSong(id: songId)
                vm.applyUpdatedSong(updated)
            } catch {
                AppLogger.library.error(
                    "Failed to refresh song on practice open: \(error.localizedDescription)"
                )
            }
        }
    }
