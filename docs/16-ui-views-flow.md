...
6. **Song Practice** — Full-screen, toggleable layout between two modes:
   ...
   - **Progress sheet** (toolbar button, chart icon): Shows score history chart (dates on X-axis), practice frequency bar chart (day/week/month toggle), overall best score, and per-phrase score breakdown. Tapping a phrase row sets up an A-B loop around that phrase (with breathing room), dismisses the sheet, and seeks to the phrase start — without auto-playing. User presses play to start the loop. The sheet also owns the **Source** section (Studio/Draft): when the song has only one variant, it shows the active source plus a "Process with <other>" button that calls `POST /v1/songs/:id/variants` and polls until the new variant is `READY`; when both variants are `READY`, it renders a segmented picker that calls `PATCH /v1/songs/:id/active-variant` (switching reloads the active variant's stems and pitch data from the variant-keyed cache). The sheet also exposes an **Edit reference…** action that opens the Reference Editor (warns that saving resets all scores for the song). The practice view refetches the song on `onAppear`, so variants that flipped to `READY` while the user was away show up without needing a library refresh.
   - **Reference Editor** (full-screen cover from Progress sheet or long-press on practice piano roll): Local-only per-song editor over the active variant's reference pitch frames. Canvas piano roll with drag-to-select range, tap-to-seek, two-finger pan, and zoom buttons. Multi-layer overlay shows the Edited result (bold amber, 3pt) on top of faded Base and other-variant lines (35% opacity, 1pt); a toggleable legend in the top-right corner controls layer visibility. Range tools in a 5-column grid: **Use Studio / Use Draft** (swap source for the range, requires alternate variant), **Despike** (max-jump-semitones slider), **Mute**, **Fill**, **Sing** (record voice over the selected range — 3-2-1 countdown then live YIN capture), **+1 st / -1 st**, **Reset**. **Draw** tool with **Replace** and **Additive** modes (additive only fills unvoiced gaps), snap-to-semitone and smooth toggles. Playback bar with play/pause and time display. Undo/redo, op list with swipe-to-delete. Save persists a `PitchEditScript` to `Documents/pitch-edits/<songId>.json`, wipes all scores + best take for the song, writes the merged frames to cache for immediate reflection in practice, and invalidates any prior cache entry. When opened from practice, the editor scrolls to the current playback time; when opened via long-press, it also pre-selects a ±2s range around the long-pressed point. Library cards show an "Edited" badge for songs with a saved script.
...
| **Long press (~1s)**      | Shows a context menu with options: **Loop Phrase** (finds nearest phrase within ±2s and sets up an A-B loop) and **Edit Reference** (opens the Reference Editor scrolled to that time with a ±2s range pre-selected). Haptic feedback on iOS. |
...
IDLE → [touch] → TOUCHING (pause, start 1s timer)
  ...
  TOUCHING → [1s elapsed] → LONG PRESS (show context menu: loop / edit)
...
- Test piano roll gestures: touch → playback pauses; swipe → graph scrolls with momentum → playback resumes from new position; long-press ~1s → context menu with "Loop Phrase" and "Edit Reference"
