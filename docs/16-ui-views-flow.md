...
6. **Song Practice** — Full-screen, toggleable layout between two modes:
   ...
   - **Reference Editor** (full-screen cover from Progress sheet): Local-only per-song editor over the active variant's reference pitch frames. Canvas piano roll + A/B time-range sliders + playback scrub. Range tools: **Use Studio / Use Draft** (swap source for the range, requires alternate variant), **Despike** (max-jump-semitones slider), **Mute**, **±8ve**. **Draw** tool with **Replace** and **Additive** modes (additive only fills unvoiced gaps), snap-to-semitone and smooth toggles. Undo/redo, op list with swipe-to-delete. Save persists a `PitchEditScript` to `Documents/pitch-edits/<songId>.json`, wipes all scores + best take for the song, and invalidates the merged-frame cache. Library cards show an ✏️ "Edited" badge for songs with a saved script.
