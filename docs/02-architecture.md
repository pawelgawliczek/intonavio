...
- Consistent ID format: CUIDs prefixed by type (`usr_`, `song_`, `stem_`, `sess_`, `ex_`, `exa_`). Clients can identify resource type from the ID.

### Module Boundary Rules (NestJS)

- One module = one domain aggregate. `SongModule` owns songs and their stems. `SessionModule` owns sessions.
- Cross-module communication via exported services only. `JobModule` can inject `SongService` because `SongModule` exports it. Never import a repository or entity from another module directly.
- No circular dependencies. If A needs B and B needs A, extract the shared concern into a new module C.
- Webhook module is a thin router. It validates the payload and delegates to the owning module. No business logic in webhook handlers.
