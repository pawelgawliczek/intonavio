...
```mermaid
flowchart TD
    A[Mic buffer arrives<br/>1024 samples] --> B[Run YIN algorithm]
    B --> C{Confidence > 0.8?}
    C -->|No| D[Mark frame as<br/>unvoiced / silence]
    C -->|Yes| E[Get detected frequency<br/>in Hz]

    E --> F[Get current playback<br/>time T seconds]
    F --> G[Binary search reference<br/>pitch at time T]
    G --> H{Reference frame<br/>voiced?}

    H -->|No| I[Skip comparison<br/>no reference note]
    H -->|Yes| J[Compute cents deviation<br/>1200 × log2 detected/reference]
...
One semitone = 100 cents. A deviation of ±50 cents means the singer is halfway to the wrong note.
