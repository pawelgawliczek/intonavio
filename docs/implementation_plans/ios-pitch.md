...
| File                                       | LOC | Purpose                                                                                                                |
| ------------------------------------------ | --- | ---------------------------------------------------------------------------------------------------------------------- |
| `Audio/Pitch/ReferencePitchFrame.swift`    | ~45 | `ReferencePitchFrame` and `ReferencePitchData` Codable structs matching pYIN worker output                             |
| `Audio/Pitch/PitchDataDownloader.swift`    | ~70 | Downloads pitch JSON from R2 via presigned URL, caches to `~/Library/Caches/pitch/{songId}/reference.json`             |
| `Audio/Pitch/ReferencePitchStore.swift`    | ~80 | Loads frames, provides O(1) lookup by time (direct index: `time / hopDuration`), range queries for piano roll          |
...
| `Features/Practice/PracticeViewModel+Loop.swift`  | ~40  | Extracted A-B loop check task from PracticeViewModel (keeps main file under 300 lines)                                                                      |
...
| File                                                  | Change                                                                                                                                                                                                                                                                                 |
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Features/Practice/PracticeViewModel.swift`           | Add properties: `pitchDetector`, `referenceStore`, `scoringEngine`, `detectedPoints`, `isPitchReady`, `layoutMode`, `visualizationMode`, `transposeSemitones`, `lastDetectedMidi`, `lastDetectionTimestamp`. Computed: `transposedMidiMin/Max`. Loop logic extracted to `+Loop.swift`. |
| `Features/Practice/SongPracticeView.swift`            | Replace `pitchPlaceholder` with `PianoRollView`. Pass `transposedMidiMin/Max` and `transposeSemitones`. Add layout toggle button. Video height driven by `layoutMode`.                                                                                                                 |
...
| `Features/Practice/ControlsBarView.swift`             | Add layout toggle, visualization mode picker, and transpose picker (Menu with `TransposeInterval.allCases`, accent-highlighted when non-zero).                                                                                                                                         |
