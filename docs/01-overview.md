...
| **Stem Separation**            | Split audio into vocals + instrumental via two interchangeable sources: **Studio** (external StemSplit API, premium quality, costs credits) and **Draft** (in-house BS-Roformer worker, free, runs on the host). Each song can hold one variant per source side by side; the user picks which is active. |
...
| **Toggleable Practice Layout** | Switch between lyrics mode (lyrics panel 35%, piano roll 65%) and video mode (YouTube 40%, piano roll 60%)                                                                                                                                                                                               |
