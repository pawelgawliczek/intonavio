...
struct YINDetector {
    ...
    func detect(
        ...
        let diff = differenceFunction(
            buffer, halfLen: halfLen
        )
        ...
        let refined = parabolicInterpolation(
            cmnd, tau: bestTau
        )
...
private extension YINDetector {
    ...
    func differenceFunction(
        ...
        for tau in 1..<halfLen {
            ...
            // diff[tau] = dot(delta, delta) = sum of squares
            var sum: Float = 0
    ...
    /// Step 4: Parabolic interpolation around the best tau
